...
const int N = 1000005, seed1 = 233, mod = 19260817, seed2 = 797, mod2 = 10000019;
//string s, str;
char s[N],str[N * 2];
ll Hash[2*N], base[2*N], Hash2[2*N], base2[2*N];
int len, lens, cnt;
namespace fastIO {
...
//unordered_map<ll, int> hmp;
pair<ll, int> ppp[N];
vector<int> vecvec[N];
inline bool cmp(const vector<int>& a, const vector<int>& b) {
    return a[0] < b[0];
}
inline bool cmp2(const pair<ll,int>& a, const pair<ll,int>& b) {
...
int main() {
	...
	for(register int i = 1; i <= len; ++i) {
		Hash[i] = (Hash[i - 1] * seed1 % mod + str[i] - 'a' + mod) % mod;
		Hash2[i] = (Hash2[i - 1] * seed2 % mod2 + str[i] - 'a' + mod2) % mod2;
	}
	for(register int i = 1; i <= len; ++i) {
		base2[i] = (base2[i-1]*seed2) % mod2;
		base[i] = (base[i - 1] * seed1) % mod;
	};
	int tmp, tmp2;
	ll ttt = 0;
	for(register int i = 1; i <= lens; ++i) {
		tmp = get(i, i + lens - 1);
		tmp2 = get2(i, i + lens - 1);
		ttt = ((ll)tmp << 32) + (ll)tmp2;
        ppp[i] = make_pair(ttt, i-1);
	}
	
	sort(ppp+1, ppp + lens+1);
    for(register int i = 1; i <= lens; ++i) {
        if(ppp[i].first != ppp[i - 1].first) cnt++;
        vecvec[cnt].push_back(ppp[i].second);
    }
    //println(cnt);
    printf("%d\n", cnt);
    sort(vecvec+1,vecvec+cnt+1, cmp);
	int lenn;
...
//		Out(lenn);
		...
		for(register int j = 0; j < lenn; ++j) {
            printf("%d%c", vecvec[i][j], j == lenn - 1?'\n':' ');
			// print(vecvec[i][j]);
			// if(j == lenn - 1) print('\n');
			// else print(' ');
		}
